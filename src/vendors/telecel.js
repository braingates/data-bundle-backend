...
export const sendTelecel = async (order) => {
  ...
  try {
    ...
    if (!process.env.TELECEL_VENDOR_URL) {
      throw new Error("TELECEL_VENDOR_URL not configured");
    }

    if (!process.env.TELECEL_API_KEY) {
      throw new Error("TELECEL_API_KEY not configured");
    }
    ...
    console.log("🔑 TELECEL URL:", process.env.MTN_VENDOR_URL);
    ...
    console.log("📥 TELECEL RESPONSE STATUS:", response.status);
    console.log("📥 TELECEL RESPONSE DATA:", response.data);
